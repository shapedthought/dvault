...
Each `dvault` subcommand maps to its own module in `src/` (`init.rs`, `add.rs`, `commit.rs`, `log.rs`, `diff.rs`, `changes.rs`, `report.rs`, `cat.rs`, `stats.rs`, `show.rs`, `checkout.rs`, `export.rs`, `tag.rs`, `status.rs`, `remove.rs`, `branch.rs`, `switch.rs`, `merge.rs`; the `config` command lives in `config_cmd.rs`, and `graph.rs` backs `log --graph`). `revparse.rs` resolves a ref (tag-or-hash) to a commit id and also exposes `file_bytes` — the shared `<file>` / `<commit> <file>` → bytes resolver used by `cat` and `changes`. `main.rs` is a clap (derive) entrypoint that dispatches to them. `config.rs` (distinct from `config_cmd.rs`) is the config.toml model; `vault.rs` handles discovery and path relativization. `identity.rs` resolves the commit author across **env vars (`DVAULT_USER_NAME`/`DVAULT_USER_EMAIL`) → per-user global config (`~/.config/dvault/config.toml`) → vault config → OS username** — env is highest precedence so shared-vault collaborators and containers (ephemeral home, no passwd entry) attribute correctly. `commit`/`merge` build their `author_*` from `identity::resolve`; `config --global` reads/writes the global file; a bare `config user.name` reports the effective value + its source. Four shared layers underpin the commands:
...
- **`refs.rs`** — branch refs and `HEAD` as plain files (`.dvault/HEAD` → `ref: refs/heads/<branch>`; `.dvault/refs/heads/<branch>` → tip commit id). A branch is "unborn" until its first commit (no ref file yet). `head_tip`, `branch_tip`, `set_branch_tip`, `current_branch`, `set_head`, `list_branches`, `delete_branch`. `branch --show-current` prints just the current branch (silent outside a vault) for `$PS1` shell-prompt hooks. `branch -d` refuses to delete an unmerged branch (tip not reachable from another branch's tip) without `-D`/`--force`, and never deletes the current branch.
...
- **Handoff (no shared vault):** `handoff.rs` (`handoff`/`receive`) is for collaborators who don't share storage. `handoff` writes a JSON slip (`.dvault/handoffs/<file>.json` internally + a portable `<stem>.handoff.json` to email) anchored to the committed version; `receive` reintegrates the returned `.docx` as a commit **authored by the recipient** (from the slip), refusing on local divergence unless `--force`. `status` shows out-for-edit files. The recipient needs only Word. Reuses `store`/`db`/`refs` (no new infra).
- **Collaboration / lock:** dvault has no server — sharing is "vault in a cloud-synced folder" (OneDrive/SharePoint), with per-machine identity (`identity.rs`) for attribution. `lock.rs` is an **advisory** lock (`.dvault/lock`, TOML: holder/email/locked_at) for coordinating concurrent edits: `commit` refuses when another identity holds it (override `--force`), `status` shows it. It's not enforceable (synced FS isn't atomic) — the blob store is append-only/safe, but `db.sqlite` + refs can conflict-copy under simultaneous commits; the lock coordinates the rare overlap in otherwise-sequential workflows.
- **Web UI:** `serve.rs` (`dvault serve`) is a small `tiny_http` (sync) server — server-rendered HTML, no framework, JS only for `confirm()`. Read views (home/file history/diff/show) query `db`/`refs`/`extract` directly and reuse `report::{diff_body_html, counts, DIFF_CSS, esc}`; write actions (commit/checkout/tag) reuse the existing command `run` fns (vault discovered from the server's CWD). Localhost by default; `--host 0.0.0.0` for Docker. No auth — localhost/trusted-network only.
- **`show`:** `show.rs` prints a commit's metadata + changed files (`db.parents_of`, `db.commit_files`), reusing `graph::decoration_for` for `(HEAD -> …, tag: …)` and `diff::show_file_diff` for the optional `--diff` per-file diffs (vs the first parent).
- **Commit graph:** `graph.rs` (`log --graph`) renders the DAG via the `renderdag` crate — it feeds `db.all_commits_with_parents()` (filtered to the reachable set) as `renderdag::Node`s and gets back one lane-glyph line per commit, then appends each commit's label + ref decorations (`HEAD -> branch`, branch names, `tag:`). `--all` spans every branch tip; otherwise just the current branch's ancestry.
...
- **Diffs:** `diff.rs::render` builds a unified diff (3 lines of context) from `similar`'s `grouped_ops` + `iter_inline_changes` (the `inline` feature must stay enabled in Cargo.toml). With color on, changed lines are red/green and the genuinely changed *words within* a line get reverse video (`\x1b[7;3Xm`); context lines are unstyled. Color gating in `use_color()`: `NO_COLOR` disables, `CLICOLOR_FORCE` forces on, else TTY detection — so piped output is plain. `diff::resolve_sides` (the `<file>` / `<from> <to> <file>` version resolution) and `diff::render` are shared with `report.rs`, which reuses them to emit a standalone HTML report (inline word highlighting via the same `iter_inline_changes`) or a Markdown ` ```diff ` block. `compare.rs` (`dvault compare a.docx b.docx`) diffs two loose files **with no vault** — it just reads both paths and calls `diff::show_file_diff`. When comparing extracted text, join paragraph lines with `diff::joined_text` (trailing newline included) — otherwise `similar` treats a change on the final paragraph as a spurious last-line replace (this bit `diff --stat`). `diff --stat` (`stat_counts`) and `stats.rs` (word counts over commits) both build on the same extraction. docx extraction (`extract/docx.rs`) collects text from `<w:t>` elements via `quick-xml`, one line per `<w:p>` paragraph. It parses not just `word/document.xml` (the body, which includes text boxes) but also headers (`word/headerN.xml`), footers, footnotes, endnotes, and comments — all the same WordprocessingML, so `parse_paragraphs` handles each. Non-body regions are concatenated in a stable order, each introduced by a `[Header]`/`[Footnotes]`/… banner line via `append_section`. Separately, `docx::tracked_changes` parses Word revision marks (`<w:ins>`/`<w:del>`, deletions carry text in `<w:delText>`) into `TrackedChange { kind, author, date, text }` for the `changes` command — distinct from snapshot diffing. Two quick-xml 0.40 gotchas the code already handles: `<w:tab/>` is emitted as a tab so adjacent runs don't glue into run-on words (the "EnglishNative" bug), and XML entities arrive as separate `Event::GeneralRef` events (not inside `Event::Text`), so they're resolved explicitly. For unsupported types, `diff` warns and shows only a file-size delta. Extraction yields `extract::Line { text, heading }` (`extract_lines`); a paragraph is a heading if its `<w:pStyle w:val>` is a `Heading*`/`Title`/`Subtitle` style (region banners count too). `diff::render` annotates each hunk with the nearest heading **above the first actual change** (`first_change` + `hunk_heading`) — git's function-context analog — shared by `diff`, `compare`, and `report` (HTML section rows + Markdown hunk labels).
